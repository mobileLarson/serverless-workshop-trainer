...
public class DynamoDBEnhancedOrderRepository implements OrderRepository {

    private static final Logger logger = LoggerFactory.getLogger(DynamoDBEnhancedOrderRepository.class);
    ...
    public Order cancelOrder(String orderId) {
        ...
        return updateItemInTable(DDB_ENH_CLIENT, orderToCancel);
    }
    ...
    // update/replace ALL fields of incoming order !
    ...
    private static Order optionalUpdateItemInTable(DynamoDbEnhancedClient ddb,
        ...
        Order updatedOrder = orderWithUpdates;
        String pk = orderWithUpdates.getOrderId();
        ...
        try {
            ...
            // put item (order) to table
            updatedOrder = orderTable.updateItem(mergeOrders(currentOrder, orderWithUpdates));
            logger.info(String.format("%s: item updated (%s))", ORDER_TABLE_NAME, pk));
        } catch (DynamoDbException ex) {
            logger.error(String.format("Error while %s updating item (%s)", ORDER_TABLE_NAME, pk));
        }
